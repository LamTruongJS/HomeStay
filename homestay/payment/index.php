...
<body>
    <?php 
      if(isset($_POST["pay"])){
        ...
        //kiểm tra xem homeStay có bị Đặt hay chưa
         $sql3="SELECT * FROM hoa_don where maHST = '$maHStay'";
         $result3=mysqli_query($conn,$sql3);
        
        for($j=0;$j<$result3->num_rows;$j++){
            $row3=mysqli_fetch_array($result3);
            if(strtotime($ngayBatDau) < strtotime($row3['ngayKetThuc'])){
                $errDate ='HomeStay đã hết  ! <br />'.'Chỉ còn sau ngày '.$row3['ngayKetThuc']; 
             } 
        }
